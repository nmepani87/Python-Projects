...
# Importing our other modules so
# we can access them too
import phonebook_main
import phonebook_gui


def center_window(self, w, h): # Passing in the tkinter frame (master) reference and the w and h
...
# Catch if the user clicks on the windows 'X' button to ensure they want to close
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"):
        # Functions to close app
        self.master.destroy()
        os._exit(0) #This releases temporary used up memory 
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # This removes any blank space before and after user's entry
    var_lname = var_lname.strip() 
    var_fname = var_fname.title() # This ensures that the first character in each word is capitalized
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #enforce the user to provide bot
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
...
def onDelete(self):
    ...
    with conn:
        ...
        # the database...cannot delete last record or we wil get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Comfirmation", "All information associated with, ({}) \nwill be permenantly deleted from the database. \nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
                    cursor.execute("""DELETE FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_select))
                onDeleted(self) # Call the function to clear all of the textboxes and the selected index of the listbox
####                onRefresh(self) # Update the Listbox of the change
                conn.commit()
        else:
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be allowed to update change for phone and emails
    # For name changes, user will need to delete the entire record and start over
    var_phone = self.txt_phone.get().strip() # Normalize data to maintain integrity
    var_email = self.txt_email.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0): # Ensure that there is data present
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: # If proposed changes are not already in the database, then proceed
                ...
                messagebox.showinfo("No changes detected","Both ({}) and ({}) \nalready exist in the database for this name.".format(var_phone,var_email,var_value))
            onClear(self)
        conn.close()
    else:
